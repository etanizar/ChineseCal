...
void PrintMonthNumber(double mnumber)
{
    ...
    ccal_printf(" [%2d]Y%1s ", month, leap);
}
                ...
                bool bSingle, int nEncoding, bool bNeedsRun)
{
    ...
    {
        ccal_printf("<ccal:month value=\"%d\" name=\"%s\" cname=\"%s\">\n",
    ...
    else
    {
        ...
        ccal_printf("%d lpts %d m gsave ct\n",
               posx, posy);
        ccal_printf("%% Month number\n%d lpts %d m gsave\n100 1 SF 1 0.9 1 K",
               ((month < 10) ? 35 : 10), -750);
        ccal_printf(" (%d) S grestore\n", month);
    }
    /* Day of week */
    char dayshort[4];
    if (pmode == 0)
    {
        ...
        {
            if (nEncoding != 'u')
                ccal_printf("%-10s", (*daynamesCH)[i]);
            else
                ccal_printf("%s   ", (*daynamesCH)[i]);
        }
        ccal_printf("\n");
    }
    else if (pmode == 1)
    {
        ccal_printf("<tr align=\"center\">\n");
        for (i = 0; i < 7; i++)
        {
            strncpy(dayshort, daynames[i], 3);
            dayshort[3] = 0;
            if (i == 0)
                ccal_printf("<td width=\"15%%\"><font color=\"#FF0000\">"
                       "%s %s</font></td>\n", dayshort, (*CHmiscchar)[15]);
            else if (i == 6)
                ccal_printf("<td width=\"15%%\"><font color=\"#00E600\">"
                       "%s %s</font></td>\n", dayshort, (*CHmiscchar)[i]);
            else
                ccal_printf("<td width=\"14%%\">%s %s</td>\n",
                       dayshort, (*CHmiscchar)[i]);
        }
        ccal_printf("</tr>\n");
    }
    else if (pmode == 2)
    {
        if (bSingle)
        {
            ccal_printf("%% Day names\n14 1 SF\n/fsc 15 def\n");
            for (i = 0; i < 7; i++)
            {
                strncpy(dayshort, daynames[i], 3);
                dayshort[3] = 0;
                if (i == 0)
                    ccal_printf("gsave 1 0 0 K\n100 2270 m (%s) S\n400 2270 m "
                           "gsave ptc %s grestore\ngrestore\n", dayshort, PSbigmchar[15]);
                else if (i == 6)
                    ccal_printf("gsave 0 0.8 0 K\n3700 2270 m (%s) S\n4000 2270 m "
                           "gsave ptc %s grestore\ngrestore\n", dayshort, PSbigmchar[i]);
                else
                {
                    int posx = i * 600 + 100;
                    ccal_printf("%d 2270 m (%s) S\n%d 2270 m gsave ptc "
                           "%s grestore\n", posx, dayshort,
    ...
    for (w = 0; w < nWeeks; w++)
    {
        if (pmode == 1)
        {
            ccal_printf("<tr align=\"right\">\n");
        }
        if (pmode == 3)
        {
            ccal_printf("<ccal:week>\n");
        }
        for (i = 0; i < 7; i++)
        {
            if (pmode == 1)
            {
                if (i == 0)
                    ...
                    ccal_printf("<td width=\"14%%\">");
            }
            if (pmode == 3)
            {
                ccal_printf("<ccal:day ");
            }
            ...
            {
                ccal_printf("value=\"%d\" cmonth=\"%d\" leap=\"%s\" cdate=\"%d\" ", dcnt, cmonth, leap, ldcnt);
            ...
            else if (bSingle)
            {
                ...
                if (i == 0)
                    ...
                    ccal_printf("gsave 0 0.8 0 K\n");
                ccal_printf("%d %d m (%2d) S\ngsave 8 0 SF ", posx, posy, dcnt);
                posx += 170;
                if (dcnt == 1 || ldcnt == 1)
                {
                    if (*leap == 'R')
                        ccal_printf("%d %d m (%dR.%d) S grestore\n%d %d m",
                            (posx + 5), (posy + 56), cmonth, ldcnt, posx,
                            (posy - 2));
                    else
                        ccal_printf("%d %d m (%d.%d) S grestore\n%d %d m",
                            (posx + 5), (posy + 56), cmonth, ldcnt, posx,
                            (posy - 2));
                }
                else
                    ccal_printf("%d %d m (%2d) S grestore\n%d %d m",
                        (posx + 5), (posy + 56), ldcnt, posx, (posy - 2));
                ccal_printf(" gsave ptc");
            }
            else
            {
                int posx = i * 19;
                ...
                if (i == 0)
                    ...
                    ccal_printf("gsave 0 0.8 0 K\n");
                ccal_printf("%d %d moveto (%2d) S\n", posx, posy, dcnt);
                posy -= 7;
                ccal_printf("%d %d moveto gsave ptc", posx, posy);
            }
            if (jdcnt != vterms[termcnt] && jdcnt != vmoons[moncnt] && !sameday)
            {
                ...
                {
                    if (nEncoding == 'a')
                        ccal_printf(" [%2d]   ", ldcnt);
                    else
                    {
                        Number2DayCH(ldcnt, nEncoding, cdayname);
                        ccal_printf(" %s   ", cdayname);
                    }
                }
                else if (pmode == 1)
                {
                    if (dcnt == 1)
                        ccal_printf(" %s", cmonname);
                    else
                        ccal_printf(" ");
                    Number2DayCH(ldcnt, nEncoding, cdayname);
                ...
                else if (pmode == 2)
                {
                    ...
                    int nlen = strlen(cdayname);
                    if (nlen > 8)
                        ccal_printf(" grestore -%d 0 rmoveto gsave ptc",
                               ((nlen - 8) / 4 * 3));
                    ccal_printf(" %s\n", cdayname);
                    if (i == 0 || i == 6)
                        ccal_printf("grestore\n");
            ...
            else if (jdcnt == vterms[termcnt])
            {
                ...
                {
                    if (nEncoding == 'a')
                        ccal_printf(" [%s]   ", jieqi[termcnt++]);
                    else
                    {
                        ccal_printf(" %s   ", (*CHjieqi)[termcnt++]);
                    }
                }
                else if (pmode == 1)
                {
                ...
                else if (pmode == 2)
                {
                    if (sameday)
                    {
                        ...
                        int nlen = strlen(cmonname);
                        if (!bSingle)
                            ccal_printf(" grestore -6 0 rmoveto gsave ptc");
                        if (!bSingle && nlen > 8)
                        {
                            double fac = 16.0 / (nlen + 8.0);
                            ccal_printf(" gsave %f 1 scale %s", fac, cmonname);
                        }
                        else
                            ccal_printf(" %s", cmonname);
                    }
            ...
            if (pmode == 3)
                ccal_printf("cmonthname=\"%s\" cdatename=\"%s\" />\n", cmonname, cdayname);
            dcnt++;
            jdcnt++;
            if (jdcnt == vmoons[moncnt])
                ldcnt = 1;
                ...
                int& pmode, bool& bSingle, int& nEncoding)
{
    ...
    for (i = 1; i < argc; i++)
    {
        if (argv[i][0] != '-')
        {
            ...
            {
                ccal_printf("ccal: Too many parameters.\n");
                return false;
            }
        }
        else if (argv[i][1] == 'x')
        ...
        {
            ccal_printf("ccal: Unrecognized option.\n");
            return false;
        }
    }
    if (pmode != 2)
...
int ccal_main(int argc, char** argv)
{
    ...
    {
        ccal_printf("ccal version %s: Displays Chinese calendar (Gregorian with Chinese dates).\n", versionstr);
        ccal_printf("Usage: ccal [-t|-p|-x] [-g|-b] [-u] [[<month>] <year>].\n");
        ccal_printf("\t-t:\tGenerates HTML table output.\n");
        ccal_printf("\t-p:\tGenerates encapsulated PostScript output.\n");
        ccal_printf("\t-x:\tGenerates XML output.\n");
        ccal_printf("\t-g:\tGenerates simplified Chinese output.\n");
        ccal_printf("\t-b:\tGenerates traditional Chinese output.\n");
        ccal_printf("\t-u:\tUses UTF-8 rather than GB or Big5 for Chinese output.\n");
        exit(1);
    }
    if (month < 1 || month > 12)
    {
        ccal_printf("ccal: Invalid month value: month 1-12.\n");
        exit(1);
    }
    if (year < 1645 || year > 7000)
    {
        ccal_printf("ccal: Invalid year value: year 1645-7000.\n");
        exit(1);
    }
    if (IsLeapYear(year))
        daysinmonth[1] = 29;
    ...
    if (pmode == 3)
    {
        ccal_printf("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
        ccal_printf("<ccal:year value=\"%d\" xmlns:ccal=\"http://ccal.chinesebay.com/ccal/\">\n", year);
    }
    if (bSingle)
    {
